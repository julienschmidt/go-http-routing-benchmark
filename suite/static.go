...
func init() {
	s1 := driver.NewBenchmark(
		driver.Static,
		"StaticNotFound",
		"Request one unavailable static endpoint",
	)
	s1.AddRoutes(staticRoutes)
	s1.AddTest(TestResponseNotFound)
	s1.AddTest(TestStaticRoutesAreStatic)
	s1.AddRequest("GET", "/notfound/this-path-is-unavailable") // Without trailing slash!

	s2 := driver.NewBenchmark(
		driver.Static,
		"StaticAll",
		"Request all static endpoints",
	)
	s2.AddRoutes(staticRoutes)
	s2.AddRequests(staticRoutes)
	s2.AddTest(TestStaticRoutesAreStatic)
	s2.AddTest(TestResponseOkWithoutOutput)
}

var staticRoutes = driver.Fixtures{
	// Cannot register root path handler because of the way http.ServeMux
	// handles unknown paths (fallback to last known directory).
	// {"GET", "/"},
